...
5. **Household gain.** What that result is worth to the household, as **income raised** and **costs cut**, always over a stated window.
...
- [Year Up](examples/year-up.md). Limited by seats, not by reach, and its earnings gain was measured over everyone offered a seat. So the page shows completion and does not multiply by it. About $1.65 in added earnings per dollar over seven years.
...
Every number takes a low, a likely, and a high, a source, and one word on its evidence, `measured` or `assumed`. A block can name a benchmark instead. The blocks are `[reach]`, `[uptake]`, `[staying]`, `[effect]`, `[income raised]`, `[costs cut]`, and `[cost]`, plus `[org reach]`, `[org uptake]`, `[org staying]`, and `[people per organization]` for the organization step. A cost says who it is charged to with `per:`. A household gain block must state its `window:`, when it was `measured at:`, and whether it `repeats:`. The same program can lose money for a household in year one and gain it by year seven, so a gain with no window is refused.
...
Forty-three tests. The arithmetic runs on hand-checkable chains, the double-count case first. The reader tests pin the refusals. The page tests pin the sentences that must never disappear. One test rebuilds every example page and compares it to the committed file byte for byte.
